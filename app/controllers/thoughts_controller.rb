class ThoughtsController < ApplicationController
    ...
    def index
        ...
        if params[:search] != nil && params[:search] != ''
            ...
            @thoughts = params[:tag_id].present? ? Tag.find(params[:tag_id]).thoughts.order("created_at DESC").page(params[:page]).per(3) : Thought.all.order("created_at DESC").page(params[:page]).per(3)
        end
